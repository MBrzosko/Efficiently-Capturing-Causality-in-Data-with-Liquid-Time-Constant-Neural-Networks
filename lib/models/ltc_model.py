...
class EnforcedLTCModel(nn.Module):
    def __init__(
        self, units: int, input_size: int, output_size: int, wiring: Wiring, ode_unfolds: int = 6, epsilon: float = 1e-8
    ):
        """
    ...
    def forward(
        ...
        for t in range(1, timespan):
            ...
            h_out, h_state = self.rnn_cell.forward(stacked_input, h_state, ts)
            h_out = self.fc(h_out)
            inputs = h_out
            outputs[t, :, :] = h_out
...
"""
The lib for the LTCCell is taken from https://github.com/mlech26l/ncps.
"""
...
class LTCCell(nn.Module):
    ...
    def _ode_solver(self, inputs, state, elapsed_time, t):
        v_pre = state
    ...
    def forward(self, inputs, states, t, elapsed_time=0.1):
        # Regularly sampled mode (elapsed time = 1 second)
        inputs = self._map_inputs(inputs)

        next_state = self._ode_solver(inputs, states, elapsed_time, t)
