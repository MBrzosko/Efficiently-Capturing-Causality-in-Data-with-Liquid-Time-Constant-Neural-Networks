...
def initialize_node(
    ...
    :return: Initialized NODE and parameter dict
    """
    ...
    # Initialize NODE
    nf = NeuralODEFunc(
        input_dim=param_dict["Input Dim"],
        output_dim=param_dict["Input Dim"],
        num_layers=param_dict["Number of Layers"],
        hidden_dim=param_dict["Units"],
    )
    node_model = EnforcedNeuralODE(odefunc=nf, input_dim=param_dict["Input Dim"], output_dim=param_dict["Output Dim"])

    return node_model, param_dict


def evaluate_node(param_dict: dict) -> None:
    """
    Evaluated the NODE model and creates a report.
    ...
    model = EnforcedNeuralODE(odefunc=nf, input_dim=param_dict["Input Dim"], output_dim=param_dict["Output Dim"])
    model.load_state_dict(torch.load(f"{param_dict["Model Name"]}.pth", weights_only=True))

    nf_val = NeuralODEFunc(
        input_dim=param_dict["Input Dim"],
        output_dim=param_dict["Input Dim"],
        num_layers=param_dict["Number of Layers"],
        hidden_dim=param_dict["Units"],
    )
    val_model = EnforcedNeuralODE(odefunc=nf_val, input_dim=param_dict["Input Dim"], output_dim=param_dict["Output Dim"])
...
def train_node(
    ...
    """
    Training loop for the NODE.

    :param node_model: Initialized NODE model.
    ...
    for epoch in range(epochs):
        ...
        for batch in data_loader:
            ...
            optimizer.zero_grad()
            output = node_model(inputs=x0_train, forcing=forcing, timespan=timespan)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(node_model.state_dict(), f"val_{model_name}.pth")
            print(f"New Best Val Loss: {best_val_loss}")

        if epoch_loss < best_loss:
            best_loss = epoch_loss
            best_epoch = epoch
            torch.save(node_model.state_dict(), f"{model_name}.pth")
